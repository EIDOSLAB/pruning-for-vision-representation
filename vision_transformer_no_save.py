...
class EncoderBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        x = self.ln_1(input)
        torch.save(torch.concat((x, x, x)), '/scratch/expl_qkv_vit.pt')
        x, attn_weights = self.self_attention(query=x, key=x, value=x, need_weights=True, average_attn_weights=False)
        torch.save(attn_weights, '/scratch/expl_attention_vit.pt')
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(image_size, int):
            torch._assert(image_size % patch_size == 0, "Input shape width indivisible by patch size!")
            image_size = (image_size, image_size)
        else:
            torch._assert(image_size[0] % patch_size == 0, "Input shape height indivisible by patch size!")
            torch._assert(image_size[1] % patch_size == 0, "Input shape width indivisible by patch size!")
...
def interpolate_embeddings(
    ...
    #new_seq_length = (image_size[0] // patch_size) ** 2 + 1
    new_seq_length = (image_size[0] // patch_size) * (image_size[1] // patch_size)  + 1
    ...
    if new_seq_length != seq_length:
        ...
        new_seq_length_1d_h = image_size[0] // patch_size
        new_seq_length_1d_w = image_size[1] // patch_size

        # Perform interpolation.
        # (1, hidden_dim, seq_l_1d, seq_l_1d) -> (1, hidden_dim, new_seq_l_1d, new_seq_l_1d)
        new_pos_embedding_img = nn.functional.interpolate(
            pos_embedding_img,
            size=(new_seq_length_1d_h, new_seq_length_1d_w),
